...
let userID = '28a006b1-65d5-4809-a303-8ea617b0d88d';
let https_proxyIPs = [];
let http_proxyIPs = [];
...
// 定义要下载的文件 URL
let myurl = 'https://zip.baipiao.eu.org/31898-1-8443.txt';
...
function createVLESSSub(userID_Path, hostName) {
    let portArray_http = [];
	//let portArray_https = [8443, 2053, 2096, 2087, 2083];
    let portArray_https = [8443];
	// Split the userIDs into an array
	...
	userIDArray.forEach((userID) => {
		...
		portArray_https.forEach((port) => {
			const commonUrlPart_https = `:${port}?encryption=none&security=tls&sni=${hostName}&fp=random&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#HTTPS-${port}`;
			const vlessMainHttps = `vless://${userID}@${hostName}${commonUrlPart_https}`;
			// For each proxy IP, generate a VLESS configuration and add to output
			https_proxyIPs.forEach((proxyIP) => {
				const vlessSecHttps = `vless://${userID}@${proxyIP}${commonUrlPart_https}-${proxyIP}`;
				//output.push(`${vlessMainHttps}-${proxyIP}`);
				//output.push(`vless://${userID}@${proxyIP}${commonUrlPart_https}-${proxyIP}`);
